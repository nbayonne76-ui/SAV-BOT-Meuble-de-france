...
import { Phone, PhoneOff, Mic, MicOff, Volume2, Loader2, Info, Clock, Camera, X } from 'lucide-react';
...
const VoiceChatWhisper = ({ onTicketCreated }) => {
  ...
  const startVoiceChat = async () => {
    try {
      ...
      await speakAndAddToHistory('assistant', 'Bonjour et bienvenue au service clientèle du groupe Mobilier de France. Nous sommes à votre écoute pour un accompagnement personnalisé. Donnez-moi votre nom, votre numéro de commande, et une description de votre problème.');
  ...
  const processAudio = async (audioBlob) => {
    ...
    try {
      ...
      else if (pendingTicket && /\b(oui|ok|validé|correct|parfait|exactement|c'est bon|tout est bon)\b/i.test(transcript)) {
        console.log('✅ Validation vocale détectée - création automatique du ticket');
      ...
      else if (pendingTicket && /\b(non|pas correct|faux|annuler|recommencer)\b/i.test(transcript)) {
      ...
      setError(`Erreur de traitement: ${err.message}. Vérifiez la console (F12) pour plus de détails.`);
    } finally {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      console.log(`📸 ${data.files.length} photo(s) uploadée(s) - le récapitulatif sera généré automatiquement`);
    ...
    if (role === 'assistant') {
      ...
      try {
        ...
        try {
          ...
          console.warn('⚠️ La lecture audio a échoué, probablement bloquée par le navigateur. Continuons sans audio.');
        ...
        {pendingTicket && (
          <div className="bg-gradient-to-r from-amber-50 to-orange-50 border-2 border-amber-400 rounded-2xl p-6 shadow-xl">
            ...
            <p className="text-gray-700 mb-6">
              Veuillez vérifier les informations ci-dessus et confirmer la création de votre demande.
            </p>
